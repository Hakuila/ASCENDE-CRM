...
create table public.audit_logs (
  ...
  -- profiles(id), não auth.users(id): mesma convenção de leads.owner_id,
  -- deals.owner_id, tasks.assigned_to, activities.author_id (ver migration
  -- 0003) — necessário para o embed actor:profiles(name) funcionar via
  -- PostgREST, que só resolve embeds com FK direta entre as duas tabelas.
...
create table public.lead_stage_history (
  ...
  -- profiles(id), mesmo motivo do actor_id em audit_logs acima.
