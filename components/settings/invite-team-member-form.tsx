...
import { inviteTeamMemberAction, type InviteMemberState } from "@/lib/settings/actions";
...
export function InviteTeamMemberForm() {
  const [state, formAction] = useFormState<InviteMemberState, FormData>(
  ...
  if (state?.success) {
    return (
      <div className="space-y-4 rounded-xl border border-green-100 bg-green-50 p-5">
        <div>
          <h3 className="font-semibold text-green-800">Pessoa adicionada!</h3>
          <p className="mt-1 text-sm text-green-700">
            Envie essas credenciais por um canal seguro — essa senha não aparece de novo.
          </p>
        </div>
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-3 rounded-xl border border-gray-100 bg-white p-4">
      ...
      <div>
        ...
        <Select id="role" name="role" defaultValue="salesperson" className="w-40">
